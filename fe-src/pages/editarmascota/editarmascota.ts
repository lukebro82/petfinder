...
const resReport = { id: "", name: "", photo: "", lat: 0, lng: 0, location: "" };

export class Reportedit extends HTMLElement {
  connectedCallback() {
    this.render();
  }
  ...
  render() {
    this.innerHTML = /*html*/ `
        ...
        <div class="homepage-pet-report"> 
            ...
            <h1 class="form-h1">Editar reporte <br> de mascota</h1>
  ...
  `;
    ...
    formName?.addEventListener("submit", (e: any) => {
      ...
      resReport.id = state.data.miPetEdit.id;
      if (
        resReport.name == "" ||
        resReport.photo == "" ||
        resReport.location == ""
    ...
    buttonDelete?.addEventListener("click", (e) => {
      ...
      const photoURL = petToDeleted["photoURL"];
      state.deletePet(id, photoURL).then((res) => {
        Router.go("/mascotas-reportadas");
      });
    });
...
customElements.define("editar-mascota", Reportedit);
